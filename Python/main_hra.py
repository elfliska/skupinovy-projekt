...
# Umí vykreslovat grafiku, číst klávesnici, myš, přehrávat zvuky atd.
import pygame
...
def save_result(username, points, hits, game_mode, duration):
    """
    Uloží výsledek ukončené hry do SQLite databáze.

    username  = jméno hráče
    points    = počet bodů (góly) hráče 1
    hits      = počet odrazů od pálky hráče 1
    game_mode = "PVP" nebo "BOT"
    duration  = délka hry v sekundách
    """
    conn = get_connection()   # Otevřeme spojení s databází (z db/models.py)
    cur  = conn.cursor()      # Vytvoříme kurzor pro spouštění SQL příkazů

    # Vložíme hráče do tabulky 'player'
    # "INSERT OR IGNORE" = pokud hráč se stejným jménem již existuje, přeskočíme
    cur.execute("INSERT OR IGNORE INTO player (username) VALUES (?)", (username,))

    # Zjistíme ID hráče z databáze (potřebujeme ho pro propojení tabulek)
    cur.execute("SELECT player_id FROM player WHERE username = ?", (username,))
    player_id = cur.fetchone()[0]   # fetchone() vrátí první řádek, [0] = první sloupec

    # Vložíme záznam o hře do tabulky 'game'
    cur.execute("""
        INSERT INTO game (game_mode, player1_id, duration_seconds)
        VALUES (?, ?, ?)
    """, (game_mode, player_id, duration))   # ? jsou bezpečné zástupné hodnoty (ochrana před SQL injection)

    game_id = cur.lastrowid   # ID právě vloženého záznamu (automaticky přiřazené databází)

    # Vložíme skóre do tabulky 'score' a propojíme s hráčem a hrou přes jejich ID
    cur.execute("""
        INSERT INTO score (points, hits, player_id, game_id)
        VALUES (?, ?, ?, ?)
    """, (points, hits, player_id, game_id))

    conn.commit()   # Potvrdíme transakci = fyzicky zapíšeme změny do souboru databáze
    conn.close()    # Zavřeme spojení s databází (dobré zvyky = vždy zavírat)
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.QUIT:
                ...
                save_result(player_name, geek1Score, geek1Hits, mode, duration)  # Uložíme výsledek
                pygame.quit()
                quit()
