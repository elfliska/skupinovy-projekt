...
from db.models import get_connection, create_tables, get_now
import os
...
def save_result(username, geek1Score, geek2Score, hits, game_mode, duration):
    ...
    cur = conn.cursor()
    cur.execute("""
        INSERT INTO hry (datum, delka_sekund, rezim, hrac1, skore1, hrac2, skore2, vítez)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    """, (get_now(), duration, game_mode, username, geek1Score, p2_name, geek2Score, winner))
    conn.commit()
    conn.close()
    print(f"✅ Uloženo: {username} {geek1Score} : {geek2Score} {p2_name} | Vítěz: {winner} | Délka: {duration}s")
